...
describe('icinga2', :type => :class) do
  on_supported_os.each do |os, facts|
    ...
    context "#{os} with constants => { foo => [ 4247, NodeName, foo in foobar ] }" do
      let(:params) { { :constants => {'foo' => ['4247', 'NodeName', 'foo in foobar']} } }

      it { is_expected.to contain_file('/etc/icinga2/constants.conf')
        .with_content(/^const foo = \[ 4247, NodeName, "foo in foobar", \]\n/) }
    end


    context "#{os} with constants => { foo => NodeName + bar(4247, NodeName, foo in foobar) - 1}" do
      let(:params) { { :constants => {'foo' => 'NodeName + bar(4247, NodeName, foo in foobar) - 1'} } }

      it { is_expected.to contain_file('/etc/icinga2/constants.conf')
        .with_content(/^const foo = NodeName \+ bar\(4247, NodeName, "foo in foobar"\) - 1\n/) }
    end


    context "#{os} with constants => { foo => { -key1 => 4247, key2 => NodeName, key3 => 1m, key4 => foo in foobar } }" do
      let(:params) { { :constants => {'foo' => { '-key1' => '4247', 'key2' => 'NodeName', 'key3' => '1m', 'key4' => 'foo in foobar'}} } }

      it { is_expected.to contain_file('/etc/icinga2/constants.conf')
        .with_content(/^const foo = \{\n\s+"-key1" = 4247\n\s+key2 = NodeName\n\s+key3 = 1m\n\s+key4 = "foo in foobar"\n\}\n/) }
    end


    context "#{os} with constants => { foo => { bar => {-key1 => 4247, baz => {key2 => NodeName, key3 => [1m, 3h, 2d]}, key4 => foo in foobar} } }" do
      let(:params) { { :constants => {'foo' => { 'bar' => {'-key1' => '4247', 'baz' => {'key2' => 'NodeName', 'key3' => ['1m','3h','2d']}, 'key4' => 'foo in foobar'}}} } }
...
#      .that_comes_before('Class[icinga2::config]') }
  ...
  context "Windows 2012 R2 with constants => { foo => [ 4247, NodeName, foo in foobar ] }" do
    let(:params) { { :constants => {'foo' => ['4247', 'NodeName', 'foo in foobar']} } }
  ...
  context "Windows 2012 R2 with constants => { foo => NodeName + bar(4247, NodeName, foo in foobar) - 1}" do
    let(:params) { { :constants => {'foo' => 'NodeName + bar(4247, NodeName, foo in foobar) - 1'} } }
  ...
  context "Windows 2012 R2 with constants => { foo => { -key1 => 4247, key2 => NodeName, key3 => 1m, key4 => foo in foobar } }" do
    let(:params) { { :constants => {'foo' => { '-key1' => '4247', 'key2' => 'NodeName', 'key3' => '1m', 'key4' => 'foo in foobar'}} } }
  ...
  context "Windows 2012 R2 with constants => { foo => { bar => {-key1 => 4247, baz => {key2 => NodeName, key3 => [1m, 3h, 2d]}, key4 => foo in foobar} } }" do
    let(:params) { { :constants => {'foo' => { 'bar' => {'-key1' => '4247', 'baz' => {'key2' => 'NodeName', 'key3' => ['1m','3h','2d']}, 'key4' => 'foo in foobar'}}} } }
