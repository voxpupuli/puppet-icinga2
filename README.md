...
Certificate from 'ca_host' using the icinga2 ticket mechanism. In case the 'ticket_salt' has
been configured the ticket_id will be generated by the module in a custom function that imitates
the icinga ticket generation.  The 'ticket_id' parameter can be used to directly set an ticket_id.
...
##### `ticket_id`
If a ticket_id is give it will be used instead of generating an ticket_id using the ticket_salt option.
The ticket_id will only be used when requesting a certificate from the ca_host in case the pki is set to 'icinga2'.
